...
Future<LNURLPageResult?> handleAuthRequest(BuildContext context, LnUrlAuthRequestData reqData) async {
  return promptAreYouSure(context, null, LoginText(domain: reqData.domain)).then((permitted) async {
    if (permitted == true && context.mounted) {
      ...
      try {
        ...
        if (resp is LnUrlCallbackStatus_Ok) {
          ...
          return LNURLPageResult(protocol: LnUrlProtocol.Auth, error: texts.lnurl_payment_page_unknown_error);
        }
      } catch (e) {
