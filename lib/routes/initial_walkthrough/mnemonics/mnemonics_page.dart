...
class MnemonicsPage extends StatefulWidget {
  ...
  const MnemonicsPage({Key? key, required this.mnemonics, this.readOnly = false}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: AutoSizeText(
          (widget.readOnly) ? texts.mnemonics_confirmation_title : texts.backup_phrase_generation_write_words,
        ),
      ),
      body: MnemonicSeedList(mnemonicsList: _mnemonicsList),
      bottomNavigationBar: SingleButtonBottomBar(
        text:
            (widget.readOnly) ? texts.admin_login_dialog_action_ok : texts.backup_phrase_warning_action_next,
        onPressed: () {
          if (widget.readOnly) {
